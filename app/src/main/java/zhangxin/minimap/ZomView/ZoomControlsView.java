package zhangxin.minimap.ZomView;
...
public class ZoomControlsView extends LinearLayout implements View.OnClickListener {
    ...
    private void init(){
        //获取布局视图
        LinearLayout view=(LinearLayout) LayoutInflater.from(getContext()).inflate(R.layout.zoom_control, null);   //null
        //获取放大按钮
    ...
    private void controlZoomShow(){
        ...
        if(zoom>=maxZoomLevel){
            inBtn.setBackgroundResource(R.mipmap.zoomin_press);
            inBtn.setEnabled(false);
        }else{
            inBtn.setBackgroundResource(R.drawable.zoomin_seletor);
            inBtn.setEnabled(true);
        }

        //如果当前状态小于等于地图的最小状态，则缩小按钮失效
        if(zoom<=minZoomLevel){
            outBtn.setBackgroundResource(R.mipmap.zoomout_press);
            outBtn.setEnabled(false);
        }else{
            outBtn.setBackgroundResource(R.drawable.zoomout_seletor);
